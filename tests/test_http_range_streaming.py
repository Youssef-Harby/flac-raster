...
class TestHTTPRangeStreaming:
    ...
    def test_bbox_query_multiple_tiles(self, sample_spatial_flac):
        ...
        expected_bytes = sum(f.byte_range[1] - f.byte_range[0] + 1 for f in frames)
...
class TestLazyLoading:
    ...
    def test_metadata_only_request(self, mock_get):
        ...
        # Mock FLAC file processing to avoid mutagen dependency in test
        with patch('flac_raster.spatial_encoder.FLAC') as mock_flac:
    ...
    def test_progressive_loading_concept(self):
        ...
        assert "embedded metadata" in workflow_steps[2]
...
class TestSpatialIndexPerformance:
    ...
    def test_spatial_index_integrity(self, sample_spatial_flac):
        ...
        sorted_frames = sorted(index.frames, key=lambda f: f.byte_range[0])
        for i in range(len(sorted_frames) - 1):
            current_end = sorted_frames[i].byte_range[1]
            next_start = sorted_frames[i + 1].byte_range[0]
            assert current_end < next_start  # No overlaps
        
        # Verify total bytes calculation
        calculated_total = sum(f.byte_range[1] - f.byte_range[0] + 1 
